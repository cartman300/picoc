...
{
	ReturnValue->Val->Pointer = strpbrk((char*)Param[0]->Val->Pointer, (const char*)Param[1]->Val->Pointer);
}

void StringStrstr(struct ParseState *Parser, struct Value *ReturnValue, struct Value **Param, int NumArgs)
{
	ReturnValue->Val->Pointer = strstr((char*)Param[0]->Val->Pointer, (const char*)Param[1]->Val->Pointer);
}
